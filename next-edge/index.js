...
function createApiHandler(options) {
  ...
  return async (req, res) => {
    ...
    const response = await fetch(url, {
      method: req.method,
      headers,
      body: req.method !== "GET" && req.method !== "HEAD" ? headers.get("content-type").includes("json") ? JSON.stringify(req.body) : new URLSearchParams(req.body) : null,
      redirect: "manual"
    });
    ...
    const buf = buffer.Buffer.from(await response.arrayBuffer());
    if (buf.byteLength > 0) {
